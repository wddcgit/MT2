...
type
  TdmMT2 = class(TDataModule)
    ...
    qMT_ErrLog: TMSSQL;
    cWebSrvr: TMSConnection;
    qPLG001: TMSQuery;
    qPRT001: TMSQuery;
    procedure PrintLog(LogId, PageID : integer; PreviewLog : boolean);
...
procedure TdmMT2.PrintLog(LogId, PageID: Integer; PreviewLog : boolean);
var
  sCID , sName, sContactNo : String;
begin
    rvMT2.ClearParams;
    if PageID = 0 then //we do not know the problem ID, so find the first one
      PageID:= GetLowestPID(LogID);
    ...
    With qRptLog do
      begin
       ...
       if PageID > 0 then //theres is also
         begin
           SQL.Add(' AND b.problemid = :pid1');
           ParamByName('pid1').AsInteger:= PageID;
         end;
   ...
   //open the tracking list
   With qRPTTrackingList do
       try
         Active:= false;
         ParamByName('pid1').AsInteger:= PageID;
         Active:= true;
...
begin
     With qMT_Select do
        try
          Active:= false;
          SQL.Text:= 'SELECT shippingmemo FROM [Multitracking].dbo.tblReturns WHERE shippingmemo = :smid';
